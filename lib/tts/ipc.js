...
function voicevoxPing(port) {
  return new Promise((resolve, reject) => {
    const req = http.request(
      { hostname: VOICEVOX_HOST, port, path: '/version', method: 'GET' },
      res => {
        const chunks = []
        res.on('data', c => chunks.push(c))
        res.on('end', () => {
          const body = Buffer.concat(chunks)
            ...
            .trim()
          if (res.statusCode >= 400) {
            reject(new Error(`VOICEVOX HTTP ${res.statusCode}`))
          } else {
...
function registerTtsIpc() {
  ...
  ipcMain.handle('tts:ping', async (event, req) => {
    ...
    try {
      return { ok: true, version: await voicevoxPing(port) }
    } catch (e) {
