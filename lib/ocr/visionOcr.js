...
// 日本語の認識はmacOS 13（Darwin 22）以降。Electron 28はmacOS 10.15から動くので版も見る
function isVisionAvailable(platform, release) {
  if ((platform || process.platform) !== 'darwin') return false
  const major = parseInt(release || require('os').release(), 10)
  return major >= 22
}
