...
// E2E プローブ実行中は window を一切表示しない。開発中に probe を回すと、
// 起動のたびに最前面へ来て作業中のアプリからフォーカスを奪い、打鍵が
// プローブ側へ流れ込む（probe の結果もその都度ぶれる）。
// macOS は画面外への setPosition を可視領域へ丸めるので「画面外に置く」
// では隠せない。show:false のまま paintWhenInitiallyHidden で描画だけ回し、
// スクリーンショットは webContents.capturePage() で撮る
const isE2EProbe = !!process.env.TB_E2E_PROBE
...
const mainWindow = new BrowserWindow({
  ...
  webPreferences: {
    ...
    sandbox: false,
    // 非表示のまま probe を回すため。表示していない window の描画を
    // 止められると capturePage が真っ黒になる。
    // 通常起動には広げない（backgroundThrottling:false は最小化中も
    // タイマー・描画を全速で回し続けるので CPU とバッテリーを食う）
    ...(isE2EProbe
...
if (isE2EProbe) {
  // Dock アイコンを出すと macOS がアプリを activate してしまう
  if (app.dock && app.dock.hide) app.dock.hide()
  mainWindow.setSkipTaskbar(true)
  // 'activate'(Dock クリック等) の show() でも前面に出させない
  mainWindow.on('show', () => mainWindow.hide())
}
