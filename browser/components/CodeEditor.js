...
import { squeezeCjkSpacing } from 'browser/lib/squeezeCjkSpacing'
  ...
  updateDefaultKeyMap() {
    ...
    this.defaultKeyMap = CodeMirror.normalizeKeyMap({
      ...
      [translateHotkey(hotkey.prettifyMarkdown)]: cm => {
        ...
        // 増えるので、出力を通してから詰める
        const formattedText = squeezeCjkSpacing(formattedTextDetails.formatted)

        // 詰めた分だけカーソルもずれる。カーソルより前を同じ規則で詰めて
        // 長さを測り直す
        const formattedCursorPos = squeezeCjkSpacing(
          formattedTextDetails.formatted.slice(
            0,
